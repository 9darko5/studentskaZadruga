package org.unibl.etf.bp.connectionPool;
...
public class ConnectionPool {
	...
	private ConnectionPool() {
		...
		try {
			...
			for (int i = 0; i < preconnectCount; i++) {
				Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/studentska_zadruga?useSSL=false",
						"root", "12369");
	...
	private void readConfiguration() {
		ResourceBundle bundle = PropertyResourceBundle.getBundle("org.unibl.etf.bp.connectionPool.ConnectionPool");
	...
	public synchronized Connection checkOut() throws SQLException {
		...
		if (freeConnections.size() > 0) {
			...
			if (connectCount < maxConnections) {
				conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/studentska_zadruga?useSSL=false",
						"root", "12369");
				usedConnections.add(conn);
				connectCount++;
			} else {
